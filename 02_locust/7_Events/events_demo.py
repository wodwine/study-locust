...
def sla(environment, **kwargs):
    if environment.stats.total.fail_ratio > 0.01:
        logging.error("Test 1 failed due to failure ratio > 0.01%")
...
def sla(environment, **kwargs):
    if environment.stats.total.fail_ratio > 0.01:
        logging.error("Test 2 failed due to failure ratio > 0.01%")
        environment.process_exit_code = 1
        print(environment.process_exit_code)
    else:
        environment.process_exit_code = 0
...
# locust -f 7_Events/events_demo.py --config locust.conf
